...
“改善驾驶起停手感” → 明确开发分支或直接主干、确认设计边界 → 必要的 Plan → 实现并同步系统文档、早期验证 → 大功能验收与收束同步 → 按约定合入并检查整合结果。

开工前说明分支选择与理由，已有约定直接沿用；多步优化默认使用开发分支，小修可直接主干。设计与计划可以先行，当前实现描述随行为变化同步提交，验收记录绑定受测版本。使用分支时，文档随代码一起合入；合入主干不自动代表正式入口已采用或验收通过。具体规则见 [AGENTS.md](AGENTS.md#开工前的分支约定)。
